...
export default function Status({ id, status, reload }) {
  ...
  const handleClick = (status) => {
    router.patch(
      route('eventSchedules.setStatus', id),
      { status },
      { onSuccess: () => reload() }
    );
  };
  ...
  return (
    <Menu>
      ...
      >
        <div>{statusText}</div>
        {auth.can.edit.schedule && <ChevronDownIcon className="h-3" />}
      </Menu.Button>
      ...
      {auth.can.edit.schedule && (
        <Menu.Items
          ...
          style={styles.popper}
          {...attributes.popper}
          className="z-30 flex flex-col overflow-auto border rounded-lg shadow-lg bg-zinc-100 border-zinc-300"
        >
          {statuses?.map((status) => (
            <Menu.Item
              key={status.id}
